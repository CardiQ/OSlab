...
static void handle_syscall(trapframe *tf)
{
  ...
  // tf point regs which store the arguement
  long syscalret = do_syscall(tf->regs.a0, tf->regs.a1, tf->regs.a2, tf->regs.a3, tf->regs.a4, tf->regs.a5, tf->regs.a6, tf->regs.a7);
  // a0 return value
  tf->regs.a0 = syscalret;
}
...
void handle_user_page_fault(uint64 mcause, uint64 sepc, uint64 stval)
{
  ...
  case CAUSE_STORE_PAGE_FAULT:
    ...
    { // 来自 #include "memlayout.h"
      void *pa = alloc_page();
      user_vm_map((pagetable_t)current->pagetable, ROUNDDOWN(stval, PGSIZE), PGSIZE, (uint64)pa, prot_to_type(PROT_WRITE | PROT_READ, 1));
      // map_pages参数：current当前物理页面的首地址，stval要被映射的逻辑地址, PGSIZE建立映射的区间长度，pa要被映射的首地址，访问权限，RoundDown防止页表中产生碎片
    }
    break;
  default:
