...
void *user_va_to_pa(pagetable_t page_dir, void *va) {
  ...
  //PGSHIFT在riscv.h
  uint64 pa = lookup_pa(page_dir,(uint64)va);  //va对应的pa页面地址
  if (pa) 
    return (void *)(pa + ((uint64)va & ((1<<PGSHIFT) -1)));  //算页偏移
  else return NULL; 
}
